...
export function auditPage({ html, pageUrl, response, redirects = [] }) {
  ...
  const twitter = {
    card: clean($('meta[name="twitter:card"]').attr('content')),
    title: clean($('meta[name="twitter:title"]').attr('content')),
    description: clean($('meta[name="twitter:description"]').attr('content')),
    image: clean($('meta[name="twitter:image"]').attr('content'))
  };
  const hreflang = $('link[rel="alternate"][hreflang]').map((_, node) => ({
    lang: clean($(node).attr('hreflang')),
    href: absoluteUrl($(node).attr('href'), pageUrl)
  })).get().filter((item) => item.lang && item.href);
  const pagination = {
    prev: absoluteUrl($('link[rel="prev"]').first().attr('href'), pageUrl),
    next: absoluteUrl($('link[rel="next"]').first().attr('href'), pageUrl)
  };
  const bodyText = clean($('body').text());
  const paragraphTexts = $('p').map((_, node) => clean($(node).text())).get().filter(Boolean);
  ...
  $('img').each((_, node) => {
    ...
    const pictureSources = el.parents('picture').first().find('source').map((__, source) => ({
      srcset: clean($(source).attr('srcset')) || null,
      sizes: clean($(source).attr('sizes')) || null,
      type: clean($(source).attr('type')) || null,
      media: clean($(source).attr('media')) || null
    })).get().filter((item) => item.srcset);
    images.push({
      src, alt: alt ?? null, title: el.attr('title') || null,
      width: width ?? null, height: height ?? null, loading,
      decoding: el.attr('decoding') || null,
      srcset: el.attr('srcset') || null,
      sizes: el.attr('sizes') || null,
      pictureSources: pictureSources.slice(0, 8)
    });
  ...
  const buttons = $('button,[role="button"],input[type="submit"],input[type="button"]').map((_, node) => clean($(node).text() || $(node).attr('value') || $(node).attr('aria-label'))).get().filter(Boolean);
  ...
  const linkStats = {
    internal: links.filter((link) => { try { return new URL(link.href).origin === origin; } catch { return false; } }).length,
    external: links.filter((link) => { try { return new URL(link.href).origin !== origin; } catch { return false; } }).length,
    nofollow: links.filter((link) => /(?:^|\s)nofollow(?:\s|$)/i.test(link.rel || '')).length,
    emptyText: links.filter((link) => !clean(link.text)).length
  };
  const h1Text = headings.find((heading) => heading.level === 1)?.text || '';
  const content = analyzeContent({ bodyText, paragraphs: paragraphTexts, title, h1: h1Text, links, buttons });
  ...
  $('form').each((_, formNode) => {
    ...
    form.find('input,select,textarea').each((__, node) => {
      ...
      formFields += 1;
      currentFields += 1;
      if (el.is('[required]') || clean(el.attr('aria-required')).toLowerCase() === 'true') { requiredFields += 1; currentRequired += 1; }
      const id = clean(el.attr('id'));
      const hasExplicitLabel = id ? $(`label[for="${id.replace(/"/g, '\"')}"]`).length > 0 : false;
      const hasImplicitLabel = el.parents('label').length > 0;
      const hasAria = Boolean(clean(el.attr('aria-label')) || clean(el.attr('aria-labelledby')));
      if (!hasExplicitLabel && !hasImplicitLabel && !hasAria) { unlabeledFields += 1; currentUnlabeled += 1; }
    ...
    formSummaries.push({ action: absoluteUrl(form.attr('action') || pageUrl, pageUrl), method: clean(form.attr('method') || 'get').toUpperCase(), fields: currentFields, requiredFields: currentRequired, unlabeledFields: currentUnlabeled });
  });
  const directContact = {
    phoneLinks: links.filter((link) => /^tel:/i.test(link.href)).length,
    emailLinks: links.filter((link) => /^mailto:/i.test(link.href)).length,
    whatsappLinks: links.filter((link) => /(?:wa\.me|api\.whatsapp\.com|whatsapp\.com)/i.test(link.href)).length
  };
  const trustLinks = links.filter((link) => privacyPattern.test(linkHaystack(link)) || termsPattern.test(linkHaystack(link)) || claimsPattern.test(linkHaystack(link))).length;
  const uxSignals = { forms, fields: formFields, requiredFields, unlabeledFields, formSummaries: formSummaries.slice(0, 12), trustLinks, ...directContact };
  ...
  const technologies = detectTechnologies({ html, $, response });
  ...
  return {
    ...
    rawHtmlBytes: Buffer.byteLength(String(html || ''), 'utf8'),
    title,
    description,
