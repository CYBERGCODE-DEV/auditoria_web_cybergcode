...
import { aggregateTechnologies, buildTechnologyProfile } from './technology-detection.js';
...
function auditProfileFingerprint(config) {
  const profile = {
    ...
    modules: Object.fromEntries(Object.entries(config.modules || {}).sort(([a], [b]) => a.localeCompare(b)))
  };
  return crypto.createHash('sha256').update(JSON.stringify(profile)).digest('hex').slice(0, 16).toUpperCase();
}
...
export async function runAuditFromCrawl({
  ...
  const [imageInspection, browser, speed, infrastructure, crux, aiReviewResult] = await Promise.all([
    ...
    (config.modules.infrastructure || config.modules.security) ? runDomainInfrastructureAudit(target, { headers:crawl.pages[0]?.headers || {} }).catch((error) => ({ status: 'unavailable', error: clip(error?.message || error), findings: [] })) : Promise.resolve({ status:'skipped', findings:[] }),
  ...
  const technologyProfile = buildTechnologyProfile({ pages:crawl.pages, browser });
  ...
  const id = `AUD-${crypto.randomUUID().toUpperCase()}`;
  const profileFingerprint = auditProfileFingerprint(config);
  ...
  return {
    meta: {
      ...
      dataIntegrity: {
        ...
        sources: [
          ...
          ...(infrastructure.registration?.status === 'measured' ? ['ICANN/IANA RDAP registration data'] : []),
          ...(infrastructure.hosting?.status === 'measured' ? ['DNS / IP RDAP / ASN hosting evidence'] : [])
        ]
      },
      consistency: {
    ...
    pages: crawl.pages.map((page) => ({
      url: page.url,
      status: page.status,
      rawHtmlBytes: page.rawHtmlBytes,
    ...
    modules: {
      ...
      domainRegistration: config.modules.infrastructure ? (infrastructure.registration?.status || 'unavailable') : 'skipped',
      hosting: config.modules.infrastructure ? (infrastructure.hosting?.status || 'unavailable') : 'skipped',
      ...
      frontendComposition: technologyProfile.status
    }
  };
}
