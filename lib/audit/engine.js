...
export async function runAudit({ url, maxPages = 12, pageSpeed = true, stableMode = true }) {
  ...
  return {
    meta: {
      ...
      dataIntegrity: {
        ...
        note: 'No se crean métricas, hallazgos ni porcentajes para módulos sin una fuente real disponible.',
        sources: [
          'HTTP responses', 'Raw HTML crawler',
          ...(browser.status === 'measured' ? ['Rendered DOM / Chromium', 'axe-core / computed styles / screenshots'] : []),
          ...(speed.status === 'measured' ? ['Google PageSpeed Insights / Lighthouse'] : []),
      ...
      consistency: {
        ...
        pageSpeedAggregation: stableMode ? `median-${STABLE_PAGESPEED_SAMPLES}` : 'single-run',
        cacheTtlMinutes: stableMode ? Math.round(STABLE_CACHE_TTL_SECONDS / 60) : 0,
        note: stableMode ? 'Perfil fijo, rastreo determinista, mediana PageSpeed y reutilización temporal del resultado.' : 'Medición en vivo sin reutilizar auditorías anteriores.'
      }
    },
    company: COMPANY,
    scores,
    ...
    performance: { status: speed.status, mobile: speed.mobile, desktop: speed.desktop, error: speed.error, stability: speed.stability },
    infrastructure,
    peru,
    iso,
