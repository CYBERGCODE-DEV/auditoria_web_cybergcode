...
export const AUDIT_PROFILE_VERSION = 'CG-STABLE-2';
...
export function makeAuditCacheKey({ url, maxPages = 12, pageSpeed = true, stableMode = true, aiReview = false, auditMode = 'complete', modules = {}, devices = {}, engineVersion = '0.0.0' }) {
  const payload = JSON.stringify({
    target: canonicalTarget(url),
    maxPages: Number(maxPages),
    pageSpeed: Boolean(pageSpeed),
    stableMode: Boolean(stableMode),
    aiReview: Boolean(aiReview),
    auditMode,
    modules,
    devices,
    engineVersion,
