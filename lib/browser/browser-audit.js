...
export async function runBrowserAudit(target, { devices = { mobile: true, desktop: true }, accessibility = true, visual = true } = {}) {
  ...
  try {
    ...
    await page.setUserAgent('Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36 CYBERGCODE-Audit/0.11');
      ...
      const cssAudit = (() => {
        ...
        const walkRules = (rules) => {
          for (const rule of [...(rules || [])]) {
            ...
            if (/:hover/.test(selectorText)) states.hover = true;
            if (/:focus/.test(selectorText)) states.focus = true;
            if (/:focus-visible/.test(selectorText)) states.focusVisible = true;
            if (/:disabled/.test(selectorText)) states.disabled = true;
            if (/:checked/.test(selectorText)) states.checked = true;
