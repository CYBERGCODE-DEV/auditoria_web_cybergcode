...
export async function runBrowserAudit(target) {
  await assertPublicUrl(target);
  let browser;
  try {
    browser = await launchAuditBrowser();
    const page = await browser.newPage();
    await page.setUserAgent('Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36 CYBERGCODE-Audit/0.6');
    ...
    const axe = await runAxeAudit(page, finalUrl);

    const desktopVisual = await inspectViewport(page, { width: 1366, height: 768, deviceScaleFactor: 1, isMobile: false, hasTouch: false });
    const desktopShot = await page.screenshot({ type: 'jpeg', quality: 48, fullPage: false, encoding: 'base64' });
    const mobileVisual = await inspectViewport(page, { width: 390, height: 844, deviceScaleFactor: 1, isMobile: true, hasTouch: true });
    const mobileShot = await page.screenshot({ type: 'jpeg', quality: 48, fullPage: false, encoding: 'base64' });
    ...
    const merged = {
      ...
      contrast: mobileVisual.contrast,
      responsive: mobileVisual.responsive,
      network,
      auditProfile: {
        id: 'CG-STABLE-1',
        ...
        desktopViewport: { width: 1366, height: 768, deviceScaleFactor: 1 },
        mobileViewport: { width: 390, height: 844, deviceScaleFactor: 1 },
        cacheDisabled: true,
        reducedMotion: true
      },
      screenshots: {
        desktop: compactScreenshot(desktopShot, { width: 1366, height: 768 }),
        mobile: compactScreenshot(mobileShot, { width: 390, height: 844 })
      }
    };
