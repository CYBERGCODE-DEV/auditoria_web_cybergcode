...
import { authConfigured, requireUser, updateAuthUser } from '../auth/supabase.js';
...
export async function requireJobRateLimit(req, res, jobId, { scope = 'job', cost = 1 } = {}) {
  if (authConfigured()) {
    const user = await requireUser(req,res,{ roles:['admin','analyst'] });
    if (!user) return false;
  }
  const token = String(req.headers?.['x-cybergcode-job-token'] || '');
  const identity = crypto.createHash('sha256').update(`${req.cybergcodeUser?.id || 'legacy'}:${jobId}:${token}`).digest('hex').slice(0, 32);
...
export async function requireAuditAccess(req, res, { scope = 'audit', cost = 1 } = {}) {
  if (authConfigured()) {
    const user = await requireUser(req,res,{ roles:['admin','analyst'] });
    if (!user) return false;
    const enforceSubscriptions = process.env.CYBERGCODE_ENFORCE_SUBSCRIPTIONS !== '0';
    if (enforceSubscriptions && !user.isAdmin && ['audit','large-audit'].includes(scope)) {
      const subscription = user.subscription || {};
      if (!['active','trialing'].includes(subscription.status)) {
        res.status(402).json({ error:'Tu cuenta no tiene una suscripción activa.', code:'SUBSCRIPTION_REQUIRED' });
        return false;
      }
      if (!subscription.auditLimit || subscription.auditUsed >= subscription.auditLimit) {
        res.status(429).json({ error:'Alcanzaste el límite de auditorías del periodo.', code:'AUDIT_QUOTA_EXHAUSTED', limit:subscription.auditLimit || 0, used:subscription.auditUsed || 0 });
        return false;
      }
      try {
        const updated = await updateAuthUser(user.id,{ subscription:{ ...subscription,auditUsed:subscription.auditUsed + 1 } });
        req.cybergcodeUser = updated;
      } catch (error) {
        res.status(503).json({ error:'No se pudo reservar la cuota de auditoría.', code:'AUDIT_QUOTA_UNAVAILABLE' });
        return false;
      }
    }
    ...
    return requireScopedRateLimit(res,{ scope, identity:`${user.organizationId}:${user.id}`, cost, limit });
  }
...
export function auditPersistenceAllowed(user = null) {
  if (authConfigured()) return Boolean(user?.id);
