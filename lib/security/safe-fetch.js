...
async function pinnedRequest(input, options = {}) {
  ...
  return await new Promise((resolve, reject) => {
    const request = client.request(url, {
      ...
      if (declared > maxBytes) { incoming.destroy(); reject(new Error('La respuesta excede el límite de análisis permitido.')); return; }
      const chunks = [];
      let bytes = 0;
...
export async function safeFetch(input, options = {}) {
  ...
  const headers = {
    'user-agent':'CYBERGCODE-Web-Audit/0.16.1 (+https://cybergcode.com)',
