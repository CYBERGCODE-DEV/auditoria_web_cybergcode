...
export async function createLargeAuditJob({ url, maxPages = 100, auditMode = 'complete', modules = {}, devices = {}, pageSpeed = true, stableMode = true, aiReview = false, actor = null }) {
  const requested = Math.min(Math.max(Number(maxPages) || 25, 1), 500);
  ...
  const job = {
    ...
    ownerUserId: actor?.id || 'legacy',
    organizationId: actor?.organizationId || 'legacy',
    status: 'crawling',
