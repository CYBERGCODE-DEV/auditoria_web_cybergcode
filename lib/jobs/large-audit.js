...
async function finalizeLargeAuditJobUnlocked(id, actor = null) {
  ...
  try {
    ...
    result.meta.largeAudit = {
      ...
      note: 'HTML/SEO se rastrea mediante un trabajo con progreso verificable; Chromium adicional usa una muestra heurística de plantillas y no sustituye la puntuación principal.'
    };
    result.pages = compactLargePages(result.pages || []);
    if (actor) result.meta.owner = { organizationId:actor.organizationId, userId:actor.id };
    try {
      if (!auditPersistenceAllowed(actor)) throw Object.assign(new Error('anonymous-persistence-disabled'), { expected:true });
      const stored = await persistAuditResult(result,{ actor });
    ...
    appendJobEvent(job, 'completed', 'Auditoría completada y resultado persistido temporalmente.');
    await setJob(job);
...
export async function finalizeLargeAuditJob(id, actor = null) {
  const lock = await acquireJobLock(`large-audit:${id}`, 360000);
  if (!lock.acquired) {
    const error = new Error('Otro worker está procesando este job. Reintenta en unos segundos.');
    error.retryable = true;
    error.job = publicJob(await getJob(id));
    throw error;
  }
  try {
    return await finalizeLargeAuditJobUnlocked(id,actor);
  } finally {
