...
import { inspectHosting, lookupDomainRegistration } from './domain-registration.js';
...
async function probeDkim(hostname) {
  ...
  const probes = await Promise.all(selectors.map(async (selector) => {
    ...
    const values = flattenTxt(records).filter((value) => {
      if (!/v=DKIM1/i.test(value)) return false;
      const key = value.match(/(?:^|;)\s*p\s*=\s*([^;\s]+)/i)?.[1] || '';
...
export async function runDomainInfrastructureAudit(target, { headers = {} } = {}) {
  const safe = await assertPublicUrl(target);
  const hostname = safe.hostname.toLowerCase();
  const url = safe.href;

  const [a, aaaa, ns, mx, txtRows, caa, dnssec, dmarcRows, mtaStsRows, tlsRptRows, tlsInfo, dkim, registration, hosting] = await Promise.all([
    ...
    tlsProbe(hostname),
    probeDkim(hostname),
    lookupDomainRegistration(hostname),
    inspectHosting(hostname, { headers })
  ]);
  ...
  if (registration.status === 'measured' && Number.isFinite(registration.daysRemaining)) {
    if (registration.daysRemaining < 0) findings.push(finding(RULES.infrastructure.domainExpired, 'El registro del dominio aparece vencido', url, `RDAP informa una fecha de expiración superada hace ${Math.abs(registration.daysRemaining)} día(s).`, 'El dominio puede entrar en gracia, suspensión o eliminación si el registro no se renueva.', 'RDAP'));
    else if (registration.daysRemaining <= 30) findings.push(finding(RULES.infrastructure.domainExpiring, 'El registro del dominio vence próximamente', url, `RDAP informa ${registration.daysRemaining} día(s) restantes.`, 'Una renovación tardía puede interrumpir DNS, correo y disponibilidad del sitio.', 'RDAP'));
  }
