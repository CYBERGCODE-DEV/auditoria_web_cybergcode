...
export function attachReportAuthorization(audit) {
  ...
  if (configured.length < 32) {
    audit.meta.reportAuthorization = { status:'unavailable', version:1, reason:'CYBERGCODE_REPORT_SECRET debe tener al menos 32 caracteres' };
    return audit;
  }
...
export function verifyReportAuthorization(audit) {
  ...
  if (configured.length < 32) return { ok:false, code:'REPORT_SIGNING_NOT_CONFIGURED', message:'La exportación PDF requiere CYBERGCODE_REPORT_SECRET de al menos 32 caracteres.' };
