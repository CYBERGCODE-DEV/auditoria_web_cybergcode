...
export async function buildAuditPdf(audit) {
  ...
  let brandLogo = null;
  try {
    const logoBytes = await fs.readFile(new URL('../../public/assets/cybergcode-logo.png', import.meta.url));
  ...
  if (brandLogo) {
    const scaled = brandLogo.scale(0.105);
    page.drawImage(brandLogo, { x: 48, y: 720, width: scaled.width, height: scaled.height });
  } else {
