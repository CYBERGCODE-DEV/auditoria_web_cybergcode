...
export function publicUser(user) {
  ...
  const auditLimit = Math.max(0,Number(user.app_metadata?.audit_limit || 0));
  const auditUsed = Math.max(0,Number(user.app_metadata?.audit_used || 0));
  return {
    id:user.id,
    email:user.email || '',
    ...
    subscription:{
      plan:String(user.app_metadata?.plan || 'none'),
      period:String(user.app_metadata?.billing_period || 'none'),
      status:String(user.app_metadata?.subscription_status || 'inactive'),
      currentPeriodEnd:user.app_metadata?.current_period_end || null,
      auditLimit,
      auditUsed,
      auditRemaining:auditLimit > 0 ? Math.max(0,auditLimit-auditUsed) : 0,
      bonusDays:Math.max(0,Number(user.app_metadata?.bonus_days || 0)),
      cancelAtPeriodEnd:Boolean(user.app_metadata?.cancel_at_period_end),
      paymentState:String(user.app_metadata?.payment_state || 'not_configured')
    },
...
export async function resolveRequestUser(req) {
  ...
  const issuedAt = (() => { try { return Number(JSON.parse(Buffer.from(token.split('.')[1] || '', 'base64url').toString('utf8'))?.iat || 0); } catch { return 0; } })();
  const validAfter = raw?.app_metadata?.session_valid_after ? Math.floor(new Date(raw.app_metadata.session_valid_after).getTime()/1000) : 0;
  if (raw && validAfter && issuedAt && issuedAt < validAfter) return null;
...
export async function updateAuthUser(userId, changes = {}) {
  ...
  const subscription = changes.subscription && typeof changes.subscription === 'object' ? changes.subscription : null;
  if (changes.role || changes.organizationId || subscription || changes.revokeSessions) {
    ...
    body.app_metadata = {
      ...
      ...(subscription ? {
        plan:['none','manual','monthly','annual'].includes(subscription.plan) ? subscription.plan : 'none',
        billing_period:['none','monthly','annual'].includes(subscription.period) ? subscription.period : 'none',
        subscription_status:['inactive','trialing','active','past_due','cancelled'].includes(subscription.status) ? subscription.status : 'inactive',
        current_period_end:subscription.currentPeriodEnd || null,
        audit_limit:Math.min(100000,Math.max(0,Number(subscription.auditLimit)||0)),
        audit_used:Math.min(100000,Math.max(0,Number(subscription.auditUsed)||0)),
        bonus_days:Math.min(3650,Math.max(0,Number(subscription.bonusDays)||0)),
        cancel_at_period_end:Boolean(subscription.cancelAtPeriodEnd),
        payment_state:String(subscription.paymentState || 'manual').slice(0,40)
      } : {}),
      ...(changes.revokeSessions ? { session_valid_after:new Date().toISOString() } : {})
    };
  }
...
export async function resendActivation(email, redirectTo) {
  return supabaseRequest('/resend', { method:'POST', body:{ type:'signup', email, options:{ email_redirect_to:redirectTo } } });
}
