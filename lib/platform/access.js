...
export function platformAccessConfigured() {
  return Boolean(String(process.env.CYBERGCODE_PLATFORM_KEY || '').trim());
}
...
export function requirePlatformAccess(req, res) {
  if (!platformAccessConfigured()) {
    res.status(503).json({ error:'CYBERGCODE_PLATFORM_KEY no está configurada en Vercel.', code:'PLATFORM_KEY_NOT_CONFIGURED' });
    return false;
  }
