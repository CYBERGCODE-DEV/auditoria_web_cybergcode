...
export function buildAuditSnapshot(audit) {
  ...
  return {
    ...
    auditMode:audit?.meta?.auditConfig?.mode || audit?.meta?.mode || null,
    profileFingerprint:audit?.meta?.consistency?.fingerprint || null,
    auditConfig:audit?.meta?.auditConfig || null,
...
export function compareAuditSnapshots(before, after) {
  const beforeFingerprint = before?.profileFingerprint || null;
  const afterFingerprint = after?.profileFingerprint || null;
  if (!beforeFingerprint || !afterFingerprint || beforeFingerprint !== afterFingerprint) {
    const error = new Error('Las auditorías usan perfiles distintos y no pueden compararse de forma fiable.');
    error.code = 'INCOMPATIBLE_AUDIT_PROFILES';
    error.details = {
      before: { fingerprint:beforeFingerprint, mode:before?.auditMode || null, config:before?.auditConfig || null },
      after: { fingerprint:afterFingerprint, mode:after?.auditMode || null, config:after?.auditConfig || null }
    };
    throw error;
  }
