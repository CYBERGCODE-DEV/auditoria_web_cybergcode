...
export function databaseUrl() {
  ...
  if (['postgres','postgresql','neon','pg'].includes(explicit)) {
    return env('SUPABASE_DB_URL') || env('POSTGRES_URL') || env('DATABASE_URL');
  }
  return env('MYSQL_URL') || env('MARIADB_URL') || env('SUPABASE_DB_URL') || env('POSTGRES_URL') || env('DATABASE_URL');
}
...
export function databaseConfigured() {
  return ['mysql','postgres'].includes(databaseProvider()) && Boolean(databaseUrl()) && !databaseConfigurationError();
}
...
function databaseConfigurationError() {
  ...
  if (!rawUrl) return 'Conexión SQL no configurada. Para Supabase define DB_PROVIDER=postgres y SUPABASE_DB_URL con la URI del Transaction pooler.';
  let decodedUrl = rawUrl;
  try { decodedUrl = decodeURIComponent(rawUrl); } catch {}
  if (/(?:\bREGION\b|PROJECT[-_ ]?REF|YOUR[-_ ]?PASSWORD|\[YOUR-PASSWORD\]|<[^>]+>|CONTRASE(?:Ñ|N)A)/i.test(decodedUrl)) {
    return 'SUPABASE_DB_URL contiene valores de ejemplo. Copia la URI real desde Supabase > Connect > Transaction pooler y reemplaza la contraseña; no uses REGION, PROJECT_REF ni [YOUR-PASSWORD] literalmente.';
  }
  try {
    const parsed = new URL(rawUrl);
    if (!parsed.hostname) return 'SUPABASE_DB_URL no contiene un servidor válido.';
    if (provider === 'postgres' && !parsed.username) return 'SUPABASE_DB_URL no contiene el usuario PostgreSQL. Copia la URI completa desde Supabase > Connect > Transaction pooler.';
    if (provider === 'postgres' && !parsed.password) return 'SUPABASE_DB_URL no contiene la contraseña de la base de datos. Reemplaza el marcador de contraseña dentro de la URI del Transaction pooler.';
  } catch {
    return 'La conexión SQL no es una URI válida. Copia la cadena completa sin espacios desde Supabase > Connect > Transaction pooler.';
  }
...
function databaseRuntimeError(error) {
  const message = String(error?.message || error || 'Error desconocido al conectar con la base de datos.');
  if (/ENOTFOUND|EAI_AGAIN|getaddrinfo/i.test(message)) {
    return 'No se pudo resolver el servidor SQL. Copia la URI real desde Supabase > Connect > Transaction pooler y verifica que no contenga REGION ni PROJECT_REF.';
  }
  if (/password authentication failed|28P01|authentication failed|access denied/i.test(message)) {
    return 'Supabase rechazó las credenciales SQL. Verifica la contraseña de la base de datos dentro de SUPABASE_DB_URL; no uses la contraseña de tu cuenta de Supabase.';
  }
  if (/timeout|ETIMEDOUT|CONNECT_TIMEOUT|connection terminated|ECONNREFUSED/i.test(message)) {
    return 'La conexión SQL no respondió a tiempo. Verifica que uses el Transaction pooler de Supabase y que la URI, el puerto y la contraseña sean correctos.';
  }
  if (/certificate|self signed|tls|ssl/i.test(message)) {
    return 'No se pudo establecer la conexión TLS con la base de datos. Verifica la URI del pooler y conserva SSL habilitado.';
  }
...
async function getPostgresSql() {
  if (!postgresSqlPromise) {
    postgresSqlPromise = import('postgres').then(({ default:postgres }) => postgres(databaseUrl(), {
      max:Math.min(Math.max(Number(env('DB_POOL_LIMIT')) || 3,1),10),
      idle_timeout:20,
      connect_timeout:Math.min(Math.max(Math.ceil((Number(env('DB_CONNECT_TIMEOUT_MS')) || 10_000) / 1000),2),30),
      prepare:false,
      ssl:env('DB_SSL').toLowerCase() === 'false' ? false : 'require'
    })).catch((error) => {
...
export async function dbQuery(statement, params = []) {
  ...
  if (provider === 'postgres') {
    ...
    return sql.unsafe(pgPlaceholders(statement), params);
  }
...
function providerLabel(provider = databaseProvider()) {
  ...
  if (provider === 'postgres') {
    ...
    if (/supabase\.(com|co)$|pooler\.supabase\.com$/i.test(host)) return 'Supabase PostgreSQL';
    if (/neon\.tech$/i.test(host)) return 'Neon PostgreSQL';
    return 'PostgreSQL';
  }
...
function databaseDescriptor(extra = {}) {
  ...
  return {
    ...
    driver: provider === 'mysql' ? 'mysql2' : provider === 'postgres' ? 'postgres.js' : null,
...
export async function platformDatabaseStatus() {
  ...
  if (!databaseConfigured() || configurationError) {
    const reason = configurationError || 'Base de datos no configurada. Define DB_PROVIDER y DATABASE_URL (o SUPABASE_DB_URL) para activar proyectos, histórico y comparativas.';
  ...
  try {
    ...
    return databaseDescriptor({ configured:true, ready:false, reason:databaseRuntimeError(error) });
  }
}
